...
STUDY_SIDE_TO_MOVE_DIAGNOSTIC_MARKDOWN = """# Why Side To Move Is Not Trusted

This chess-study export writes the machine-readable diagnostic audit to
`why_side_to_move_not_trusted.json`. The full crop-linked Markdown and HTML
audit is generated by the auto-flow artifact pipeline.
"""
STUDY_SIDE_TO_MOVE_DIAGNOSTIC_HTML = """<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <p>This chess-study export writes the machine-readable diagnostic audit to
  <code>why_side_to_move_not_trusted.json</code>. The full crop-linked Markdown
  and HTML audit is generated by the auto-flow artifact pipeline.</p>
</body>
</html>
"""
QUALITY_THRESHOLDS = {
...
def run_chess_study_export(
    ...
    *,
    ...
    if config.html and config.html.is_file():
        ...
        _write_study_side_to_move_diagnostic_report(config.out, diagrams.get("diagrams", []) or [], source_gate=source_gate)
    render_study_html(
        config.out,
        structure=structure,
        positions=positions,
...
def _attach_pdf_side_marker_evidence_to_study_diagrams(
    ...
    *,
    ...
    if not diagrams:
        ...
        _write_study_side_to_move_diagnostic_report(out, [], source_gate=None)
        return summary
...
def _write_study_side_to_move_diagnostic_report(
    out: Path,
    diagrams: list[Mapping[str, Any]],
    *,
    source_gate: Mapping[str, Any] | None,
    ...
    reports_dir = out / "reports" / "chess_fen"
    reports_dir.mkdir(parents=True, exist_ok=True)
    report = build_side_to_move_diagnostic_report(diagrams, source_gate=source_gate, artifact_root=out)
    _write_json(reports_dir / "why_side_to_move_not_trusted.json", report)
    (reports_dir / "why_side_to_move_not_trusted.md").write_text(
        STUDY_SIDE_TO_MOVE_DIAGNOSTIC_MARKDOWN,
        encoding="utf-8",
    )
    (reports_dir / "why_side_to_move_not_trusted.html").write_text(
        STUDY_SIDE_TO_MOVE_DIAGNOSTIC_HTML,
        encoding="utf-8",
    )

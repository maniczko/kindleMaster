...
| 2. Publication analysis | `publication_analysis.py`, `ml_features.py`, `ml_route_model.py` | Detects publication class and route hints for books, reports, scans, magazines, training/chess-like inputs, and DOCX. The local JSON route model runs in `shadow` by default and reports `route_decision`; `assist` requires an explicit flag and confidence gates. | `test_publication_analysis.py`, `test_ml_route_model.py`, `test_publication_pipeline.py`, `test_premium_corpus_smoke.py` |
...
| 8. Runtime quality gate | `app_runtime_services.py`, `epub_premium_scoring.py`, `ml_quality_verifier.py`, `quality_state_service.py` | Scores the final EPUB bytes after optional heading repair, records `premium_scoring` and local `ai_quality_verification`, and keeps default `draft` conversions downloadable but blocked for publication. | `test_app_runtime_services.py`, `test_quality_state_service.py`, `test_ml_quality_verifier.py` |
...
| 10. Offline learning loop | `ml_feedback.py`, `scripts/build_ml_datasets.py`, `scripts/retrain_all.py`, `model_registry.py`, `kindlemaster.py ml feedback` | Logs local operator/runtime feedback as append-only JSONL and optionally extends `reports/ml/datasets/route_examples.jsonl` with explicitly labeled route examples. Each `ml dataset` run also writes immutable versioned datasets under `reports/ml/datasets/versions/<dataset_version>/` plus `latest_readiness.html/json` so operators can see whether training is currently allowed. `ml retrain-all` is the batch-only path for dataset build, candidate training, evaluation, corpus gate, registry-backed rollback snapshot, and optional promotion. Promotions update `models/registry.json`, append `reports/ml/promotions/promotion_history.jsonl`, and write model cards under `reports/ml/model_cards/`. No online learning or per-conversion model mutation happens here. | `test_ml_datasets.py`, `test_ml_retrain_all.py`, `test_model_registry.py`, `test_kindlemaster_entrypoint.py` |
...
| ML uncertainty | `route_decision`, `model_attribution`, `models/registry.json`, `reports/ml/datasets/completeness_report.json`, `reports/ml/datasets/latest_readiness.html` | Keep runtime in `shadow` unless assist gates pass; rebuild datasets with `python kindlemaster.py ml dataset` before training and check the latest readiness dashboard before model training or promotion. Use `python kindlemaster.py ml promote --dry-run` before mutation and `python kindlemaster.py ml rollback --model route_classifier --to-version <version>` if a promoted model must be restored. |
| Runtime quality draft | `quality_gate_mode`, `premium_scoring`, `ai_quality_verification` | Default `draft` mode allows downloading the EPUB as a review artifact but sets release verdict to blocked until an operator promotes the workflow. |
| Feedback uncertainty | `reports/ml/feedback/conversion_feedback.jsonl`, `reports/ml/datasets/feedback_route_examples.jsonl` | Treat feedback as local audit data. Include it in route datasets only with explicit valid labels and extracted features; never update model weights during conversion. |
...
- ML route, review, registry, or feedback changes: run `python kindlemaster.py ml dataset`, `python kindlemaster.py ml evaluate`, `python kindlemaster.py ml promote --dry-run`, and the ML unit tests before enabling assist experiments. For feedback-specific changes, also exercise `python kindlemaster.py ml feedback --export-dir reports\ml\feedback` against a local conversion report.

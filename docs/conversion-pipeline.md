...
| 10. Offline learning loop | `ml_feedback.py`, `scripts/build_ml_datasets.py`, `kindlemaster.py ml feedback` | Logs local operator/runtime feedback as append-only JSONL and optionally extends `reports/ml/datasets/route_examples.jsonl` with explicitly labeled route examples. Each `ml dataset` run also writes immutable versioned datasets under `reports/ml/datasets/versions/<dataset_version>/` plus `latest_readiness.html/json` so operators can see whether training is currently allowed. No online learning or route model mutation happens here. | `test_ml_datasets.py`, `test_kindlemaster_entrypoint.py` |

...
- `test`
- `audit`
- `chess` with `export-side-to-move-audit`, `validate-side-markers`, `prepare-fen-gold-corpus`, `auto-label-fen-corpus`, `import-fen-gold-labels`, `reconcile-diagram-manifest`, `join-chess-evidence`, `audit-fen-conflicts`, `import-evidence-review-queue`, and `export-evidence-review-labels` subcommands
- `chess-study` with `run-all`, `audit-current`, `extract-structure`, `segment-pages`, `detect-diagrams`, `recognize-fen`, `extract-pgn`, `link-exercises`, `validate`, `render`, `fen-review`, `build-fen-templates`, `evaluate-fen-profile`, `pgn-review`, `quality-dashboard`, `ai-fen-candidates`, `ai-pgn-candidates`, `ai-quality-eval`, `quality-baseline`, `two-crop-performance`, `preprocess-boards`, `build-square-dataset`, `train-fen-classifier`, `evaluate-fen-classifier`, `recognize-fen-local`, `evaluate-fen-ensemble`, `calibrate-fen-confidence`, `export-fen-review-corpus`, and `export-fen-corpus-manifest` subcommands; use `--quality-profile smoke|default|masterkindle` for study-export gates

Use `process` as the front-door automatic chess PDF flow when the goal is one command that runs extraction, deterministic FEN/PGN validation, safe candidate mapping, canonical reports, and strict export gating. Use `chess prepare-fen-gold-corpus` to bind manual marker evidence and a full-FEN review package to one source SHA without publishing labels. Use `chess auto-label-fen-corpus` for crop-only local/vision consensus and exception routing; automatic rows must not claim human verification or independent accuracy. Use `chess reconcile-diagram-manifest` to reconcile detector drift into a draft expected-diagram manifest; only exact fingerprint and unambiguous source-bound bbox matches may resolve automatically, while split/merge/duplicate cases remain review-only. Use `chess join-chess-evidence` to join that private draft with canonical FEN labels, their source review rows, and historical marker labels; the command may bridge the two fingerprint schemes only through exact-source one-to-one geometry and must keep unresolved evidence review-only. Use `chess audit-fen-conflicts` to bind a current run to source-matched 64-square labels, compute independent confusion matrices, and adjudicate model-template conflicts only where human-verified ground truth exists. Use `chess import-evidence-review-queue` and `chess export-evidence-review-labels` for the dedicated Supabase marker-bbox queue; do not place canonical evidence rows into the 64-square FEN label table. Then use `chess import-fen-gold-labels` to reject incomplete, mismatched, duplicate, or invalid human review exports before training. Use `chess validate-side-markers` for source-SHA-bound acceptance against the non-committed verified fixed-edition corpus; synthetic fixtures must never satisfy that gate. Use `report` and `review` for the derived auto-flow report and manual review index. Keep `chess-study` as the stage/debug backend for semantic reading order, diagram/FEN recognition, and PGN/comment extraction. Exact PDF visual layout and full page-image rendering are audit/debug aids only; use `--render-pages` explicitly when page images are needed.

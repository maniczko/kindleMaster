...
def _resolve_review_conversion_job(job_id: str) -> dict | None:
...
def convert_chess_evidence_review(job_id: str):
    _mark_timed_out_conversion_jobs()
    _cleanup_expired_conversion_jobs()
    job = _resolve_review_conversion_job(job_id)
    if not job:
        return _json_error(
            "Nie znaleziono zadania konwersji.",
            error_code=ERROR_MISSING_OUTPUT,
    ...
    if payload is None:
        return _json_error(
            "Nie znaleziono kolejki dowodow markerow.",
            error_code=ERROR_MISSING_OUTPUT,
            status_code=404,
            phase="evidence_review",
            job_id=job_id,
    ...
    response = app.response_class(
        render_chess_evidence_review_html(payload),
        mimetype="text/html",
    )
    response.headers["Cache-Control"] = "no-store, max-age=0"
    response.headers["Pragma"] = "no-cache"
...
@app.route("/convert/artifact/<job_id>/chess_evidence_review_progress", methods=["GET", "PUT"])
def convert_chess_evidence_review_progress(job_id: str):
    _mark_timed_out_conversion_jobs()
    _cleanup_expired_conversion_jobs()
    if not _resolve_review_conversion_job(job_id):
        return _json_error(
            "Nie znaleziono zadania konwersji.",
            error_code=ERROR_MISSING_OUTPUT,
            status_code=404,
...
@app.route("/convert/artifact/<job_id>/chess_evidence_review_export", methods=["GET"])
def convert_chess_evidence_review_export(job_id: str):
    ...
    response.headers["Content-Disposition"] = f'attachment; filename="marker_evidence_{job_id}.jsonl"'
    response.headers["Cache-Control"] = "no-store, max-age=0"
    return response

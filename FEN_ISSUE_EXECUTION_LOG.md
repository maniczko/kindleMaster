...
| #23 | DONE | fen-p0-05-recover-lost-strict | pending | `python -m unittest test_chess_fen_pipeline_hardening.py test_chess_auto_flow.py test_chess_fen_ml_acceptance.py`; `python -m unittest test_chess_fen_recognition.py`; `python -m py_compile pymupdf_chess_extractor.py chess_fen_hardening.py`; full `kindlemaster.py convert`; strict diff; strict regression gate | Passed | Restored strict FEN to `227/377` versus required `223/377`; accepted records have explicit side evidence (`marker=190`, `exact_label=37`) and no accepted `side_to_move_inferred`; gate status `passed`. |

...
def _is_pseudo_heading_candidate(node: Tag, text: str) -> bool:
    ...
    if any(token in class_tokens for token in {"heading", "title", "subtitle", "chapter-title", "section-title", "toc-entry"}):
        return True
    first_alpha = next((char for char in text.strip() if char.isalpha()), "")
...
def _build_non_linear_reachability_toc_entries(
    ...
    *,
    ...
    for position, chapter_path in enumerate(visible_paths, start=1):
        ...
        label_key = _magazine_key(label)
        if "sponsor" in label_key or "contact" in label_key or "kontakt" in label_key:
            continue
...
def _build_magazine_chapter_info(chapter_path: Path, *, index: int) -> dict[str, object]:
    ...
    elif image_count >= 1 and len(full_text) < 200:
        special_type = "image_stub"
    else:
        special_type = ""
    return {
        "path": chapter_path,
...
def _looks_like_magazine_non_content_info(info: dict[str, object]) -> bool:
    ...
    if special_type in {"gallery", "advertisement", "sponsored", "contact", "newsletter", "image_stub"}:
        return True
    ...
    *,
    ...
    for info in chapter_infos:
        ...
        for candidate in info["candidates"]:
            ...
            first_alpha = next((character for character in candidate_text if character.isalpha()), "")
            if first_alpha and first_alpha.islower():
                continue
...
def _fallback_magazine_non_linear_files(chapter_infos: list[dict[str, object]], *, contents_file: str) -> set[str]:
    ...
    for info in chapter_infos:
        file_name = str(info["file_name"])
        if file_name == contents_file or bool(info.get("is_contents")):
            continue
...
def _classify_magazine_feature_buckets(
    ...
    *,
    ...
    for info in chapter_infos:
        ...
        if title_key.startswith(("photo", "zdjecie", "zdjęcie")) and int(info.get("image_count") or 0) >= 1:
            extras.add(file_name)
            continue
        if _looks_like_magazine_non_content_info(info):
            extras.add(file_name)
    ...
    if not resolved_author or _is_placeholder_author(resolved_author):
        for chapter_path in chapter_paths:
            ...
            if match:
                ...
                break
    if not resolved_author or _is_placeholder_author(resolved_author):
        resolved_author = "Unknown Author" if allow_training_defaults else "Unknown"
...
def _toc_entry_href(entry: dict) -> str:
    text_key = _normalize_key(str(entry.get("text") or ""))
    if text_key in {"table of contents", "contents", "spis tresci", "spis treści", "spis treäąâ€şci"}:
        return entry["file_name"]
    if str(entry.get("file_name") or "") == "chapter_001.xhtml" and text_key == "cover":
        return entry["file_name"]
...
def _normalize_toc_entries_for_render(toc_entries: list[dict]) -> list[dict]:
    ...
    for entry in toc_entries:
        ...
        label_key = _normalize_key(label)
        file_name = str(entry.get("file_name") or "")
        if file_name == "chapter_001.xhtml" and label_key in {"table of contents", "contents", "spis tresci", "spis treści"}:
            label = "Cover"
        try:
            level = int(entry.get("level", 1) or 1)
        except (TypeError, ValueError):

...
const Icon = ({ d, size = 18, className = "" }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d={d} /></svg>;
...
const IcoCheck    = () => <Icon d="M20 6L9 17l-5-5" className="text-emerald-500" />;
const IcoCross    = () => <Icon d="M18 6L6 18M6 6l12 12" className="text-rose-500" />;
...
async function sbUpsert(table, rows) { for (let i = 0; i < rows.length; i += 100) { const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, { method: "POST", headers: sbH("resolution=merge-duplicates,return=minimal"), body: JSON.stringify(rows.slice(i, i + 100)) }); if (!r.ok) throw new Error(`Batch ${i/100+1}: ${await r.text()}`); } }
...
function buildPlan(history, weakCat) { if (!history.length) return { readiness:"Brak danych", recommendation:"Ukończ kilka prób quizu, aby wygenerować plan.", improvements:[], weeklyPlan:[] }; const l5=history.slice(0,5); const avgAcc=Math.round(l5.reduce((s,a)=>s+a.percent,0)/l5.length); const avgPace=Number((l5.reduce((s,a)=>s+a.avgResponseMs,0)/l5.length/1000).toFixed(1)); const wm={},sm={}; l5.forEach(a=>{ if(a.weakestCategory) wm[a.weakestCategory]=(wm[a.weakestCategory]||0)+1; if(a.strongestCategory) sm[a.strongestCategory]=(sm[a.strongestCategory]||0)+1; }); const weak=Object.entries(wm).sort((a,b)=>b[1]-a[1])[0]?.[0]||weakCat?.category||"Mieszane tematy"; const strong=Object.entries(sm).sort((a,b)=>b[1]-a[1])[0]?.[0]||"Wiedza ogólna"; return { readiness: avgAcc>=85?"Zaawansowany":avgAcc>=65?"Średniozaawansowany":"Buduj podstawy", recommendation: avgAcc>=85?"Gotowy na trudniejsze zestawy.":avgAcc>=65?"Dobra baza — wzmocnij słabe kategorie.":"Skup się najpierw na dokładności.", improvements: [ avgAcc<70?"Zwolnij przy trudnych pytaniach.":"Utrzymaj wysoką dokładność.", avgPace>18?"Zwiększ szybkość na łatwych pytaniach.":"Tempo jest optymalne.", `Poświęć czas na: ${weak}.` ], weeklyPlan:[ {day:"Dzień 1", task:`Przejrzyj błędy z obszaru: ${weak}.`, duration:"25m"}, {day:"Dzień 2", task:`Skupiony quiz z ${weak}.`, duration:"30m"}, {day:"Dzień 3", task:"Zestaw mieszany z czasomierzem.", duration:"20m"}, {day:"Dzień 4", task:"Podsumuj najważniejsze zasady.", duration:"20m"}, {day:"Dzień 5", task:`Szybki quiz w ${strong}.`, duration:"25m"}, {day:"Dzień 6", task:"Pełny próbny quiz wiedzy.", duration:"30m"}, {day:"Dzień 7", task:"Lekki przegląd materiału.", duration:"15m"} ] }; }
...
export default function QuizAbcdApp() {
  ...
  const askAI = useCallback(async () => {
    ...
    setTimeout(() => {
      setChatRes(`💡 Wskazówka AI: Pytanie dotyczy kategorii "${current.category}". Zwróć uwagę na słowa kluczowe w opcjach. Odpowiedź poprawna (${current.correct}) najlepiej odzwierciedla standardowe definicje.`);
  ...
  const tabs=[
    {id:"quiz", label:"Quiz", icon: <IcoBrain size={16}/>}, 
    {id:"calendar", label:"Kalendarz", icon: <IcoCalendar size={16}/>}, 
    {id:"plan", label:"Plan Nauki", icon: <IcoBook size={16}/>},
    ...(showResult?[{id:"results", label:"Wyniki", icon: <IcoTrophy size={16}/>}]:[]), 
    {id:"settings", label:"Ustawienia", icon: <IcoSettings size={16}/>}
  ];

  // ── WIDOKI (Tylko środkowa część) ──────────────────────────────────────────
  const QuizView = () => (
    <div className="flex flex-col h-full overflow-y-auto px-1 pb-8">
      {/* Pasek postępu */}
      <div className="mb-8">
        <div className="flex justify-between text-xs font-bold text-slate-400 tracking-widest uppercase mb-2">
          <span>Pytanie {idx + 1} z {total}</span>
          <span>{Math.round((idx / Math.max(total, 1)) * 100)}%</span>
        </div>
        <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
          <div className="h-full bg-indigo-500 transition-all duration-500 ease-out" style={{width:`${((idx)/Math.max(total,1))*100}%`}} />
        </div>
      </div>
      ...
      <div className="mb-8">
        <div className="text-xs font-semibold text-slate-500 mb-3 flex items-center gap-2">
          <span className="bg-slate-200 text-slate-700 px-2.5 py-1 rounded-md">{current.category || "Ogólne"}</span>
          <span className="text-slate-300">•</span>
          <span className="capitalize">{current.difficulty || "Medium"}</span>
        </div>
        <h2 className="text-2xl font-semibold text-slate-800 leading-snug">{current.question}</h2>
      </div>
      ...
      <div className="space-y-3 flex-1">
        {optionKeys.map(key => {
          ...
          // Eleganckie, minimalistyczne style dla kart odpowiedzi
          let cardClass = "border-slate-200 bg-white hover:border-indigo-400 hover:shadow-sm text-slate-700";
          let letterClass = "bg-slate-100 text-slate-500";
          let icon = null;

          if (reveal) {
            if (isCorr) { 
              cardClass = "border-emerald-500 bg-emerald-50/50 text-emerald-900 shadow-sm"; 
              letterClass = "bg-emerald-500 text-white border-transparent"; 
              icon = <IcoCheck />;
            } 
            else if (isSel && !isCorr) { 
              cardClass = "border-rose-400 bg-rose-50/50 text-rose-900 shadow-sm"; 
              letterClass = "bg-rose-500 text-white border-transparent"; 
              icon = <IcoCross />;
            } 
            else { 
              cardClass = "border-slate-100 bg-white opacity-50"; 
              letterClass = "bg-slate-100 text-slate-400"; 
            }
          }

          return (
            <button key={key} onClick={() => handleAnswer(key)} className={`w-full text-left p-4 rounded-xl border transition-all duration-200 flex items-center justify-between group ${cardClass}`}>
              <div className="flex items-center gap-4">
                <span className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center font-bold text-sm border border-slate-200 transition-colors ${!reveal ? "group-hover:bg-indigo-100 group-hover:text-indigo-600 group-hover:border-indigo-200" : ""} ${letterClass}`}>
                  {key}
                </span>
      ...
      {/* Box z wyjaśnieniem i AI */}
      {selected && (
        <div className="mt-6 bg-slate-50 rounded-xl p-5 border border-slate-200 shadow-sm animate-in fade-in slide-in-from-bottom-2">
           <div className="text-sm font-semibold text-slate-800 mb-1">{current.correct ? (selected === current.correct ? "Świetnie! Poprawna odpowiedź." : `Niestety. Poprawna odpowiedź to: ${current.correct}.`) : "Klucz niedostępny."}</div>
           <div className="text-sm text-slate-600 mb-4">{current.explanation}</div>
           
           {chatStatus === 'idle' && (
             <button onClick={askAI} className="inline-flex items-center gap-2 text-sm font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors"><IcoChat size={16}/> Zapytaj AI o detale</button>
           )}
           {chatStatus === 'loading' && <div className="text-sm font-medium text-indigo-500 animate-pulse flex items-center gap-2 px-1"><IcoChat size={16}/> Łączenie z asystentem AI...</div>}
           {chatStatus === 'loaded' && <div className="text-sm bg-white border border-indigo-100 rounded-lg p-4 text-indigo-900 mt-2 shadow-sm leading-relaxed">{chatRes}</div>}
        </div>
      )}

      {/* Dolny pasek nawigacji */}
      <div className="mt-8 pt-4 flex justify-between items-center shrink-0">
        <button onClick={prev} disabled={idx === 0} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-500 hover:text-slate-800 hover:bg-slate-200 disabled:opacity-30 disabled:hover:bg-transparent transition-colors">
          <IcoLeft size={16}/> Poprzednie
        </button>
        <button onClick={next} disabled={!selected} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold bg-slate-900 text-white hover:bg-slate-800 disabled:opacity-40 disabled:hover:bg-slate-900 transition-all shadow-sm">
          {idx === total - 1 ? "Zakończ Quiz" : "Następne"} <IcoRight size={16}/>
        </button>
      </div>
    </div>
  );
  ...
  const ResultsView = () => (
    <div className="h-full overflow-y-auto px-1 pb-10 space-y-6">
      <div className="flex items-center gap-3 mb-8">
        <div className="p-3 bg-emerald-100 text-emerald-600 rounded-2xl"><IcoTrophy size={24}/></div>
        <h2 className="text-2xl font-bold text-slate-800">Quiz Ukończony!</h2>
      </div>
      
      <div className="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm text-center">
        <div className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-2">Twój Wynik</div>
        <div className="text-5xl font-extrabold text-indigo-600 mb-2">{Math.round((score/Math.max(total,1))*100)}%</div>
        <div className="text-slate-500 font-medium">{score} poprawnych na {total} pytań</div>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {[ {l:"Średnie tempo", v:fmt(stats.avgResponseMs)}, {l:"Całkowity czas", v:fmt(stats.totalTimeMs)}, {l:"Poprawne", v:stats.correctCount, c:"text-emerald-600"}, {l:"Błędne", v:stats.incorrectCount, c:"text-rose-600"} ].map(c=>(
          <div key={c.l} className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm"><div className="text-xs font-bold text-slate-400 tracking-wider uppercase mb-1">{c.l}</div><div className={`text-2xl font-bold ${c.c || "text-slate-800"}`}>{c.v}</div></div>
        ))}
      </div>
    </div>
  );
  ...
  const CalendarView = () => (
    <div className="h-full overflow-y-auto px-1 pb-10 space-y-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Twój Kalendarz Nauki</h2>
      <div className="grid grid-cols-3 gap-4">
        {[{l:"Dni Nauki",v:Object.keys(dayMap).length},{l:"Aktualna Seria",v:streak},{l:"Wszystkie Próby",v:uniq.length}].map(c=><div key={c.l} className="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm"><div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{c.l}</div><div className="text-2xl font-bold text-indigo-600">{c.v}</div></div>)}
      </div>
      <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm mt-4">
        <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-lg text-slate-800 capitalize">{calMonth.toLocaleString("pl-PL",{month:"long",year:"numeric"})}</h3><div className="flex gap-2"><button onClick={()=>setCalMonth(m=>addM(m,-1))} className="p-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-200 transition-colors"><IcoLeft size={16}/></button><button onClick={()=>setCalMonth(m=>addM(m,1))} className="p-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-200 transition-colors"><IcoRight size={16}/></button></div></div>
        <div className="grid grid-cols-7 gap-2 text-center text-xs font-bold text-slate-400 mb-2 uppercase">{["Pn","Wt","Śr","Cz","Pt","So","Nd"].map(d=><div key={d}>{d}</div>)}</div>
        <div className="grid grid-cols-7 gap-2 text-center text-sm font-medium">
          {calDays.map(i=><div key={i.key} className={`aspect-square flex items-center justify-center rounded-xl transition-all ${i.inCurrent?"bg-slate-50 text-slate-700":"text-slate-300"} ${dayMap[i.key]?"border-2 border-indigo-500 bg-indigo-50 text-indigo-700 shadow-sm":""}`}>{i.date.getDate()}</div>)}
        </div>
      </div>
    </div>
  );
  ...
  const PlanView = () => (
    <div className="h-full overflow-y-auto px-1 pb-10 space-y-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Plan Treningowy</h2>
      <div className="p-8 bg-indigo-600 text-white rounded-3xl shadow-md">
        <div className="text-indigo-200 text-xs font-bold uppercase tracking-widest mb-2">Rekomendacja Systemu</div>
        <div className="text-2xl font-medium leading-tight">{plan.recommendation}</div>
      </div>
      <div className="space-y-3">
        {plan.weeklyPlan.map(item => (
          <div key={item.day} className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm flex items-center justify-between group hover:border-indigo-200 transition-colors">
            <div><div className="font-bold text-slate-800 mb-1">{item.day}</div><div className="text-sm text-slate-500">{item.task}</div></div>
            <div className="text-xs font-bold tracking-wide bg-slate-100 px-3 py-1.5 rounded-lg text-slate-600">{item.duration}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  const SettingsView = () => (
    <div className="h-full overflow-y-auto px-1 pb-10 space-y-6">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Ustawienia Aplikacji</h2>
      
      <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
        <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><IcoBook size={20} className="text-indigo-500"/> Baza Pytań ({questionPool.length})</h3>
        <p className="text-sm text-slate-500">{importMsg}</p>
        <button onClick={()=>fileRef.current?.click()} className="w-full flex justify-center items-center gap-2 py-3 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl font-semibold transition-colors border border-slate-200"><IcoUpload size={18}/> Wgraj własny plik (CSV / TXT)</button>
        <input ref={fileRef} type="file" accept=".csv,.xlsx,.xls,.txt" className="hidden" onChange={handleImport}/>
      </div>

      <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
        <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><IcoKeyboard size={20} className="text-indigo-500"/> Skróty Klawiaturowe</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm font-medium text-slate-600">
          <div className="flex items-center gap-3"><span className="bg-slate-100 border border-slate-200 px-2 py-1 rounded-md text-xs font-bold text-slate-800 shadow-sm">A B C D</span> Wybór odpowiedzi</div>
          <div className="flex items-center gap-3"><span className="bg-slate-100 border border-slate-200 px-2 py-1 rounded-md text-xs font-bold text-slate-800 shadow-sm">Enter</span> Następne pytanie</div>
          <div className="flex items-center gap-3"><span className="bg-slate-100 border border-slate-200 px-2 py-1 rounded-md text-xs font-bold text-slate-800 shadow-sm">R</span> Szybki restart quizu</div>
        </div>
      </div>

      <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
        <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><IcoCloud size={20} className="text-indigo-500"/> Chmura Supabase</h3>
        <div className="flex items-center gap-2 text-sm font-semibold">
          Status: <span className={SB_ENABLED ? "text-emerald-600 flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-md" : "text-slate-400 bg-slate-100 px-2 py-1 rounded-md"}>{SB_ENABLED ? "Połączono" : "Brak połączenia"}</span>
        </div>
        <p className="text-xs text-slate-500 leading-relaxed">Synchronizacja postępów wymaga ustawienia zmiennych środowiskowych podczas budowania aplikacji w Vercel.</p>
      </div>
      
      <button onClick={()=>{localStorage.removeItem(STORAGE_KEY);setHistory([]);}} className="w-full flex justify-center items-center gap-2 py-3.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-2xl font-bold transition-colors"><IcoTrash size={18}/> Zresetuj wszystkie lokalne statystyki</button>
    </div>
  );

  // ── GŁÓWNY LAYOUT (SaaS Style - 2 Kolumny) ─────────────────────────────────
  return (
    <div className="h-screen w-full flex flex-col bg-[#F8FAFC] font-sans overflow-hidden text-slate-800 selection:bg-indigo-100 selection:text-indigo-900">
      
      {/* 1. Minimalistyczny Header */}
      <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-slate-200/80 shrink-0 z-10">
        <div className="font-extrabold text-xl text-slate-900 tracking-tight flex items-center gap-2">
          <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white"><IcoBrain size={18}/></div>
          QuizApp
        </div>
        
        {/* Nawigacja Główna (Centrum) */}
        <nav className="hidden md:flex gap-2 bg-slate-100/50 p-1 rounded-xl border border-slate-200/50">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} 
              className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold transition-all rounded-lg ${activeTab === t.id ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-800 hover:bg-slate-100/80"}`}>
              {t.icon} {t.label}
            </button>
          ))}
        </nav>
        ...
        <div className="text-xs font-bold text-slate-500 bg-slate-100 px-4 py-2 rounded-xl hidden sm:flex items-center gap-2 border border-slate-200/50">
          Ukończone próby: <span className="text-slate-800 text-sm">{uniq.length}</span>
        </div>
      </header>

      {/* Nawigacja Mobilna (Tylko na małych ekranach) */}
      <nav className="flex md:hidden gap-2 p-3 bg-white border-b border-slate-200 overflow-x-auto shrink-0 z-10 custom-scrollbar">
        {tabs.map(t=><button key={t.id} onClick={()=>setActiveTab(t.id)} className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold whitespace-nowrap rounded-lg ${activeTab===t.id?"bg-indigo-50 text-indigo-700 border border-indigo-100":"text-slate-500 bg-slate-50 border border-transparent"}`}>{t.icon} {t.label}</button>)}
      </nav>

      {/* 2. Główna zawartość (Podział na sekcje) */}
      <main className="flex-1 flex overflow-hidden relative">
        
        {/* LEWY PANEL KONTROLNY (Ustawienia i statystyki sesji) */}
        {(activeTab === "quiz" || activeTab === "results") && (
          <aside className="hidden lg:flex w-80 bg-white border-r border-slate-200/80 flex-col shrink-0 overflow-y-auto shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-0">
            <div className="p-8 flex flex-col gap-10">
              
              {/* Sekcja: Ustawienia Sesji */}
              <div>
                <div className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest mb-4">Ustawienia Sesji</div>
                <div className="bg-slate-100 p-1.5 rounded-xl flex mb-4 border border-slate-200/60">
                  {[5, 10, 20, "all"].map(len => (
                    <button key={len} onClick={() => { setQuizLength(len); startQuiz(questionPool, len); }} 
                      className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${quizLength === len ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
                      {len === "all" ? "Wszystkie" : len}
                    </button>
                  ))}
                </div>
                <button onClick={() => startQuiz(questionPool, quizLength)} className="w-full py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-100 text-sm font-bold rounded-xl transition-colors flex justify-center items-center gap-2">
                  <IcoRefresh size={16}/> Rozpocznij nową sesję
                </button>
              </div>

              <div className="h-px w-full bg-slate-100"></div>

              {/* Sekcja: Statystyki Bieżące */}
              <div>
                <div className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest mb-4">Postęp Sesji</div>
                
                <div className="mb-8">
                  <div className="text-sm font-bold text-slate-700 flex justify-between mb-2">
                    <span>Ukończono</span>
                    <span className="text-indigo-600">{answeredCount} / {total}</span>
                  </div>
                  <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-500 rounded-full transition-all duration-500" style={{width:`${(answeredCount/Math.max(total,1))*100}%`}}></div>
                  </div>
                </div>

                <div className="mb-8">
                   <div className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Skuteczność</div>
                   <div className="text-4xl font-extrabold text-slate-800 tracking-tight">{answeredCount > 0 ? Math.round((stats.correctCount / answeredCount) * 100) : 0}<span className="text-2xl text-slate-400 ml-1">%</span></div>
                </div>

                <div>
                  <div className="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest mb-3">Odpowiedzi</div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center bg-emerald-50 px-4 py-3 rounded-xl border border-emerald-100">
                      <span className="text-emerald-700 text-sm font-bold flex items-center gap-2"><IcoCheck size={16}/> Poprawne</span>
                      <span className="font-extrabold text-emerald-800">{stats.correctCount}</span>
                    </div>
                    <div className="flex justify-between items-center bg-rose-50 px-4 py-3 rounded-xl border border-rose-100">
                      <span className="text-rose-700 text-sm font-bold flex items-center gap-2"><IcoCross size={16}/> Błędne</span>
                      <span className="font-extrabold text-rose-800">{stats.incorrectCount}</span>
                    </div>
                  </div>
                </div>
        ...
        {/* ŚRODKOWA SEKCJA: Aktywny Widok (Scrollowalny tylko w pionie) */}
        <div className="flex-1 overflow-y-auto bg-[#F8FAFC] custom-scrollbar">
          <div className="max-w-3xl mx-auto w-full pt-10 px-6 sm:px-10 pb-20">
      ...
      {/* Prosty kod CSS dla własnego, niewidocznego scrollbara, aby nie było "grubego paska" z boku */}
